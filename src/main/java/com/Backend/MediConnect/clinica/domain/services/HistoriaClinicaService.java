 ...
 * import
 * com.Backend.MediConnect.clinica.domain.dto.request.SignoVitalRequestDTO;
 ...
 * import
 * com.Backend.MediConnect.clinica.domain.exception.ResourceNotFoundException;
 ...
 * 
 * public HistoriaClinicaService(IHistoriaClinicaRepository
 ...
 * return
 * historiaClinicaRepository.findByPaciente_IdPaciente(paciente.getIdPaciente())
 * .orElseGet(() -> historiaClinicaRepository.save(
 * HistoriaClinica.builder().paciente(paciente).build()));
 * }
 * 
 * @Transactional
 * public AtencionMedicaResponseDTO iniciarAtencion(Long idCita, String
 * usuarioCreacion) {
 ...
 * throw new
 * BusinessException("Ya existe una atención registrada para esta cita.");
 * }
 * 
 ...
 * 
 * @Transactional
 * public void registrarSignoVital(Long idAtencion, SignoVitalRequestDTO
 * request) {
 * 
 * throw new UnsupportedOperationException(
 * "Este método pertenece al módulo antiguo. Usar ConsultaMedicaService.");
 * 
 * }
 * 
 * @Transactional
 * public DiagnosticoResponseDTO registrarDiagnostico(Long idAtencion,
 * DiagnosticoRequestDTO request) {
 ...
 * 
 * @Transactional
 * public TratamientoResponseDTO registrarTratamiento(Long idAtencion,
 ...
 * 
 * @Transactional
 * public AtencionMedicaResponseDTO cerrarAtencion(Long idAtencion,
 * AtencionMedicaCierreRequestDTO request) {
 * AtencionMedica atencion = obtenerAtencionValidada(idAtencion);
 * 
 * List<Diagnostico> diagnosticos =
 * diagnosticoRepository.findByAtencionMedica_IdAtencion(idAtencion);
 * if (diagnosticos.isEmpty()) {
 * throw new BusinessException(
 * "Debe registrar al menos un diagnóstico antes de cerrar la atención.");
 * }
 * 
 ...
 * tratamientoRepository.findByAtencionMedica_IdAtencion(idAtencion);
 * SignoVital signo = null;
 * 
 ...
 * .orElseThrow(() -> new
 * ResourceNotFoundException("Atención médica no encontrada."));
 * 
 * if (!"CERRADA".equals(atencion.getEstado())) {
 * throw new
 * BusinessException("Solo se puede generar la constancia de una atención cerrada."
 * );
 * }
 * 
 * List<Diagnostico> diagnosticos =
 ...
 * tratamientoRepository.findByAtencionMedica_IdAtencion(idAtencion);
 * 
 * byte[] pdf = atencionMedicaPdfService.generarPdf(atencion, diagnosticos,
 * tratamientos);
 * 
 ...
 * 
 * @Transactional
 * public DocumentoClinicoResponseDTO subirDocumento(Long idPaciente, Long
 * idAtencion,
 ...
 * HistoriaClinica historia = obtenerOCrear(paciente);
 * 
 * AtencionMedica atencion = idAtencion != null ?
 * obtenerAtencionValidada(idAtencion) : null;
 * 
 * Map<String, String> resultado = documentoClinicoStorageService.subir(archivo,
 * idPaciente);
 * 
 ...
 * 
 * @Transactional
 * public DocumentoClinicoResponseDTO actualizarDocumento(Long idDocumento,
 * DocumentoClinicoRequestDTO request) {
 * DocumentoClinico documento = documentoClinicoRepository.findById(idDocumento)
 * .orElseThrow(() -> new
 * ResourceNotFoundException("Documento no encontrado."));
 * 
 ...
 * 
 * @Transactional
 * public DocumentoClinicoResponseDTO reemplazarArchivoDocumento(Long
 * idDocumento, MultipartFile archivo) {
 * DocumentoClinico documento = documentoClinicoRepository.findById(idDocumento)
 * .orElseThrow(() -> new
 * ResourceNotFoundException("Documento no encontrado."));
 * 
 ...
 * }
 * 
 * public List<DocumentoClinicoResponseDTO> listarDocumentosPorPaciente(Long
 * idPaciente) {
 * HistoriaClinica historia =
 * historiaClinicaRepository.findByPaciente_IdPaciente(idPaciente)
 * .orElseThrow(() -> new ResourceNotFoundException(
 * "Este paciente no tiene historia clínica registrada."));
 * 
 * return documentoClinicoRepository.findByHistoriaClinica_IdHistoria(historia.
 * getIdHistoria()).stream()
 * .map(mapper::toResponse)
 * .toList();
 * }
 * 
 * public List<DocumentoClinicoResponseDTO> listarDocumentosPorAtencion(Long
 * idAtencion) {
 * return
 * documentoClinicoRepository.findByAtencionMedica_IdAtencion(idAtencion).stream
 * ()
 * .map(mapper::toResponse)
 * .toList();
 * }
 ...
 * "Este paciente no tiene historia clínica registrada."));
 * 
 * List<AntecedenteClinicoResponseDTO> antecedentes =
 * antecedenteClinicoRepository
 ...
 * .findByHistoriaClinica_IdHistoriaOrderByFechaAtencionDesc(historia.
 * getIdHistoria())
 * .stream()
 * .map(atencion -> {
 * SignoVital signo = null;
 * List<Diagnostico> diagnosticos = diagnosticoRepository
 * .findByAtencionMedica_IdAtencion(atencion.getIdAtencion());
 ...
 * if ("CERRADA".equals(atencion.getEstado())) {
 * throw new
 * BusinessException("No se puede modificar una atención médica ya cerrada.");
 * }
 * 
 * return atencion;
 * }
